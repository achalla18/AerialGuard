        ...
        con.execute(
            """INSERT INTO track_points
               (incident_id, track_id, timestamp, cx, cy, bbox_w, bbox_h,
                confidence, speed, avg_speed, altitude_proxy,
...
def get_stats() -> dict:
    ...
    with _conn() as con:
        ...
        top_rules = con.execute(
            "SELECT rule, COUNT(*) AS cnt FROM alerts "
            "WHERE timestamp > ? "
    ...
    return {
        ...
        "top_rules": [
            {"rule": r["rule"], "count": r["cnt"]} for r in top_rules
        ],
    }
