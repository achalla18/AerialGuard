"""
main.py — AerialGuard v2: AI Aerial Intrusion Detection

Entry point wiring all subsystems into a real-time pipeline:
  detection → analytics → behavior → threat → alerts → incidents → dashboard
...
def draw_object(frame, info: dict, trail: deque, alert_active: bool):
    ...
    if info.get("circling"):
        cv2.putText(frame, "[CIRCLING]", (x1, y_off),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.38, (0, 165, 255), 1)
        y_off += 13
    if info.get("approach_aggressiveness", 0) > 0.5:
        cv2.putText(frame, "[APPROACHING]", (x1, y_off),
...
def main():
    ...
    alert_mgr = AlertManager(
        ...
        approach_cooldown_s=al_cfg.get("rapid_approach_cooldown_seconds", 15),
        log_file=al_cfg.get("log_file", "alerts.log"),
    )
    print("  Alerts    : rule engine loaded")
    ...
    try:
        while True:
            ...
            # 3. Behavior classification + threat scoring
            ...
            # 4. Alert rules
            new_alerts = alert_mgr.process(analytics_map)
            if new_alerts:
                flash_until = now + 2.0
