...
def _generate_summary(inc: _ActiveIncident) -> str:
    ...
    if "hover" in inc.triggered_rules:
        parts.append("Hovering behaviour was flagged near the facility.")
    if "circling" in inc.triggered_rules:
        parts.append("Object performed circling manoeuvres.")
    if "rapid_approach" in inc.triggered_rules:
        parts.append("Aggressive closing approach toward the facility was detected.")
    if inc.closest_approach != float("inf") and inc.closest_approach < 200:
        parts.append(
            f"Closest approach to facility centre: {inc.closest_approach:.0f} px."
        )
    if inc.behavior_tag:
