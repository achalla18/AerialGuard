...
function renderActiveTracks(objects) {
  ...
  el.innerHTML = sorted.map(o => {
    ...
    if ((o.approach_aggressiveness || 0) > 0.5) flags.push('APPROACHING');
    ...
    return `<div class="track-card ${level}">
      ...
      <div class="track-stats-grid">
        ...
        <div class="track-stat"><div class="tsk">AGGR</div><div class="tsv">${((o.approach_aggressiveness||0)*100).toFixed(0)}%</div></div>
      </div>
...
function handleAlert(a) {
  ...
  if (banner && text) {
    const ruleLabel = a.rule.replace(/_/g, ' ').toUpperCase();
    text.textContent = `ALERT — Track T${a.track_id}  ${ruleLabel}`;
...
function renderTactical(data) {
  ...
  ctx.fillText('FACILITY', cx + 10, cy - 8);
  ...
  if (!data.tracks || !data.tracks.length) {
    ctx.fillStyle = 'rgba(74,112,144,.4)';
    ctx.font      = '14px monospace';
    ctx.fillText('No active tracks', W / 2 - 60, H / 2 + 40);
    return;
  }
  ...
  data.tracks.forEach(track => {
    ...
    // Approach indicator arrow (orange dashed line toward centre)
    if ((track.approach_aggressiveness || 0) > 0.5) {
      ...
      ctx.beginPath();
      ctx.moveTo(tx, ty);
      // draw arrow pointing ~30% toward facility centre
      const dx = cx - tx, dy = cy - ty;
      const len = Math.hypot(dx, dy) || 1;
      ctx.lineTo(tx + (dx / len) * 20, ty + (dy / len) * 20);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.globalAlpha = 1;
    }
...
function incCard(inc) {
  ...
  const chips  = rules.map(r => `<span class="rule-chip ${r}">${r.replace(/_/g,' ')}</span>`).join('');
  ...
  return `<div class="incident-card ${level}" onclick="openIncident(${inc.id})">
    ...
    <div class="inc-body">
      ...
      ${inc.behavior_tag ? `<div class="inc-stat" style="font-size:11px;margin-bottom:6px">
        Behavior: <strong>${escHtml(inc.behavior_tag.replace(/_/g,' '))}</strong></div>` : ''}
...
async function openIncident(id) {
  ...
  try {
    ...
    document.getElementById('detail-stats-tbl').innerHTML = rows([
      ...
      ['Behavior',         (inc.behavior_tag||'—').replace(/_/g,' ')],
      ...
      if (!inc.alerts?.length) {
        ...
        tl.innerHTML = inc.alerts.map(a => {
          ...
          return `<div class="tl-item">
            ...
            <span class="tl-rule">${a.rule.replace(/_/g,' ')}</span>
          </div>`;
        }).join('');
      }
    }
...
function buildCharts(points, since) {
  ...
  _updateChart(_charts.heading, mkDs('heading_volatility', tids, byTrack));
...
async function loadAlertQueue() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = '<tr><td colspan="8" class="empty">Failed to load alerts.</td></tr>';
  }
}
...
function renderAlertQueue(alerts) {
  ...
  tbody.innerHTML = alerts.map(a => {
    ...
    return `<tr id="alert-row-${a.id}">
      ...
      <td><span class="aq-rule ${a.rule}">${a.rule.replace(/_/g,' ')}</span></td>
